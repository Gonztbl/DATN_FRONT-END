...
const FoodAndDrinkPage = () => {
    ...
    const CategorySkeleton = () => (
        <div className="flex gap-3 overflow-x-auto pb-2">
            {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="flex items-center gap-2 px-6 py-3 rounded-full bg-slate-200 dark:bg-slate-800 animate-pulse min-w-[120px] h-12" />
            ))}
        </div>
    );

    const ProductCardSkeleton = () => (
        <div className="bg-white dark:bg-slate-800 rounded-2xl overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] animate-pulse flex flex-col">
    ...
    return (
        <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden font-sans bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100">
            {/* ═══════════════ HEADER ═══════════════ */}
            <header className="sticky top-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 px-4 md:px-10 py-4 shadow-sm">
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-10">
                        ...
                        <div className="flex items-center gap-2 cursor-pointer">
                            ...
                            <h2 className="text-slate-900 dark:text-white text-2xl font-black tracking-tight">SmartPay</h2>
                        </div>
                        {/* Nav */}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="hidden lg:flex items-center gap-2 px-4 py-2 bg-primary/10 border border-primary/20 rounded-full">
                            ...
                            <span className="text-primary font-bold text-sm whitespace-nowrap">{formatCurrency(balance)}</span>
                        </div>
                        {/* Search */}
                        <div className="hidden md:flex relative group">
                            <input
                                className="bg-slate-100 dark:bg-slate-800 border-none rounded-full py-2.5 pl-10 pr-4 text-sm w-64 focus:ring-2 focus:ring-primary/20 focus:bg-white dark:focus:bg-slate-700 transition-all outline-none text-slate-900 dark:text-slate-100 placeholder:text-slate-400"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    {loading ? (
                        ...
                        products.map((product) => (
                            <div
                                key={product.id}
                                className="group bg-white dark:bg-slate-800 rounded-2xl overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.12)] transition-all duration-500 cursor-pointer flex flex-col"
                            ...
                            >
                                ...
                                <div className="p-5 flex-1 flex flex-col">
                                    ...
                                    <div className="flex items-center justify-between mt-auto pt-4">
                                        <span className="text-slate-900 dark:text-white font-black text-xl">{formatCurrency(product.price)}</span>
                                        <button
                                            className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-700 hover:bg-primary hover:text-white px-4 py-2 rounded-xl text-slate-600 dark:text-slate-300 font-bold text-sm transition-all"
                        ...
                        <div className="col-span-full py-20 flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-3xl bg-white/30 dark:bg-slate-800/30">
                ...
                {totalPages > 1 && (
                    <div className="mt-12 flex items-center justify-center gap-2">
                        <button
                            ...
                            disabled={page === 1}
                            className="size-10 flex items-center justify-center rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 disabled:opacity-50 hover:border-primary hover:text-primary transition-all shadow-sm"
                        >
            ...
            {showModal && selectedProduct && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10 bg-slate-900/60 backdrop-blur-md">
                    ...
                    >
                        ...
                        <div className="w-full md:w-1/2 overflow-y-auto border-r border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900" style={{ scrollbarWidth: 'none' }}>
                            ...
                            <div className="p-8 md:p-10">
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        <h2 className="text-3xl font-black text-slate-900 dark:text-white leading-tight">{selectedProduct.name}</h2>
                                        <p className="text-primary font-black text-3xl mt-2">{formatCurrency(selectedProduct.price)}</p>
                                    </div>
                                ...
                                {selectedProduct.restaurant && (
                                    <div className="p-5 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 mb-8 flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                            <div className="size-14 rounded-xl bg-white dark:bg-slate-700 shadow-sm flex items-center justify-center border border-slate-100 dark:border-slate-600 overflow-hidden">
                                            ...
                                            <div>
                                                <h4 className="font-bold text-slate-900 dark:text-white text-lg">{selectedProduct.restaurant.name}</h4>
                        ...
                        <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col bg-slate-50 dark:bg-slate-800 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
                            ...
                            <div className="space-y-6 flex-1">
                                ...
                                <div className="space-y-5">
                                    {/* Address */}
                                    <div>
                                        <label className="block text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">Địa chỉ chi tiết</label>
                                        <div className="flex items-center bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl p-4 focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                                            <span className="material-symbols-outlined text-slate-400 text-xl mr-3">map</span>
                                            <input
                                                className="bg-transparent border-none focus:ring-0 w-full text-sm font-medium p-0 outline-none text-slate-900 dark:text-white placeholder:text-slate-400"
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">Họ & Tên</label>
                                            <div className="flex items-center bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl p-4 focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                                                <input
                                                    className="bg-transparent border-none focus:ring-0 w-full text-sm font-medium p-0 outline-none text-slate-900 dark:text-white placeholder:text-slate-400"
                                        ...
                                        <div>
                                            <label className="block text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">Số điện thoại</label>
                                            <div className="flex items-center bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl p-4 focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                                                <input
                                                    className="bg-transparent border-none focus:ring-0 w-full text-sm font-medium p-0 outline-none text-slate-900 dark:text-white placeholder:text-slate-400"
                                                    placeholder="0xxx..."
                                    ...
                                    {/* Note */}
                                    <div>
                                        <label className="block text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">Ghi chú (Tùy chọn)</label>
                                        <textarea
                                            className="w-full bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl p-4 text-sm font-medium focus:ring-2 focus:ring-primary/20 focus:border-slate-200 transition-all outline-none text-slate-900 dark:text-white placeholder:text-slate-400"
                            ...
                            <div className="mt-10 pt-8 border-t border-slate-200 dark:border-slate-700">
                                {/* Quantity */}
                                <div className="flex items-center justify-between mb-6">
                                    <span className="text-slate-500 dark:text-slate-400 font-bold">Số lượng:</span>
                                    <div className="flex items-center bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-full p-1.5 shadow-sm">
                                        <button
                                            className="size-10 rounded-full bg-slate-50 dark:bg-slate-600 text-slate-900 dark:text-white flex items-center justify-center hover:bg-slate-100 dark:hover:bg-slate-500 transition-colors"
                                            onClick={() => handleQuantityChange(-1)}
                                        >
                                            <span className="material-symbols-outlined text-lg">remove</span>
                                        </button>
                                        <span className="w-12 text-center font-black text-lg text-slate-900 dark:text-white">{quantity}</span>
                                        <button
                                            className="size-10 rounded-full bg-primary text-white flex items-center justify-center hover:bg-primary/90 transition-colors shadow-md shadow-primary/20"
                                            onClick={() => handleQuantityChange(1)}
                                        >
                                ...
                                <div className="flex items-center justify-between mb-8">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-3xl font-black text-slate-900 dark:text-white">{formatCurrency(totalPrice)}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <span className="material-symbols-outlined text-primary">verified_user</span>
                                        <span className="text-[10px] text-primary font-black uppercase tracking-tighter">Bảo mật bởi SmartPay</span>
                                    </div>
                                </div>

                                {/* Submit */}
                                <button
                                    className={`w-full text-white text-lg font-black py-5 rounded-2xl transition-all flex items-center justify-center gap-3 active:scale-[0.98] ${isRestaurantClosed ? 'bg-slate-400 cursor-not-allowed shadow-none' : 'bg-primary hover:bg-primary/90 shadow-[0_10px_30px_rgba(5,150,105,0.3)]'} disabled:bg-slate-300 disabled:shadow-none`}
