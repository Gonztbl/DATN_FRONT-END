        ...
        if (!editingRestaurant || formData.name !== editingRestaurant.name) {
            try {
                ...
                if (checkData && checkData.exists) {
                    alert(`Tên nhà hàng "${formData.name}" đã tồn tại!`);
                    return;
                }
            } catch (error) {
    ...
    return (
        <div className="relative flex h-auto min-h-screen w-full flex-col font-display bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 overflow-x-hidden md:flex-row">
            ...
            <div className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
                {/* Navigation */}
                <header className="flex h-16 shrink-0 items-center justify-between whitespace-nowrap border-b border-solid border-slate-200 dark:border-slate-800 bg-white dark:bg-background-dark px-6 py-3 lg:px-10 sticky top-0 z-10">
                    ...
                    <div className="flex flex-1 justify-end gap-4 items-center">
                        <div className="hidden sm:flex items-center gap-2 bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700">
                            ...
                            <input className="bg-transparent border-none focus:ring-0 text-sm placeholder:text-slate-500 w-40 outline-none" placeholder="Tìm kiếm nhanh..." type="text" />
                        </div>
                        <button className="material-symbols-outlined text-slate-500 hover:text-primary">notifications</button>
                        <div className="flex items-center gap-3 pl-4 border-l border-slate-200 dark:border-slate-700">
                            <div
                                className="bg-center bg-no-repeat aspect-square bg-cover rounded-full size-9 ring-2 ring-primary/20"
                                style={{
                                    backgroundImage: `url(${user?.avatar || "https://lh3.googleusercontent.com/aida-public/AB6AXuCe42CJT7zXVUqTzzIhT2LkTyZpSZJaq4y4loskWxujmHqjcnTnd8xZSYQkWlW67G8RyzTSNNStTMVUyB_MR5-u73l-gjWiOcwqiH3_dd61q3NkKop0sYKJ2b0vSs5Y_CLJVoV4AHg1wx51FhGKCazBnUpUyb5sf5rJF3WRAmbjNRDlcOHrzeyMj_8bkPR_S7VJiMqj2SM8LPHdycnByycvaAnWrm3WPYp36ZzB_Xa2dpm5u6PgLZHUNKXkLmh3p8kWLfcuI5thtmzl"})`
                                }}>
                            </div>
                            <button onClick={handleLogout} className="hidden lg:flex px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-xs font-bold rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">Đăng xuất</button>
                        </div>
                    </div>
                </header>
