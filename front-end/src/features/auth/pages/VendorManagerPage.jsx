    ...
    const handleSubmit = async (e) => {
        ...
        if (!editingCategory || formData.name !== editingCategory.name) {
            try {
                ...
                if (checkData && checkData.exists) {
                    alert(`Tên danh mục "${formData.name}" đã tồn tại!`);
                    return;
                }
            } catch (error) {
        ...
        try {
            ...
            const errorMsg = typeof error.response?.data === 'string'
                ? error.response?.data
                : error.response?.data?.message || "Có lỗi xảy ra khi lưu trữ danh mục!";
            alert(errorMsg);
        }
    };
    ...
    return (
        <div className="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 min-h-screen flex font-display">
            ...
            <main className="flex-1 flex flex-col min-w-0 bg-background-light dark:bg-background-dark h-screen overflow-y-auto w-full">
                {/* Top Navigation Bar theo giao diện ADMIN */}
                <header className="flex h-16 shrink-0 items-center justify-between whitespace-nowrap border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-6 py-3 lg:px-10 sticky top-0 z-10">

                    <div className="flex flex-1 justify-end gap-4 items-center">
                        <div className="hidden sm:block">
                            <label className="flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg px-3 h-10 w-64 border border-transparent focus-within:border-primary/50 transition-all">
                                <span className="material-symbols-outlined text-slate-400 text-xl">search</span>
                                <input class="bg-transparent border-none focus:ring-0 text-sm w-full placeholder:text-slate-400 outline-none px-2" placeholder="Tìm kiếm hệ thống..." type="text" />
                            </label>
                        </div>
                        <button onClick={handleLogout} className="flex items-center justify-center rounded-lg h-10 px-4 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 transition-colors text-sm font-bold">
