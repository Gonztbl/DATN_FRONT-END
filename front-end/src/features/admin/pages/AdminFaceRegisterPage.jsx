...
export default function AdminFaceRegisterPage() {
    ...
    const handleCapture = async () => {
        ...
        try {
            ...
            const poseLabelMap = { "front": "CHÍNH DIỆN", "left": "QUAY TRÁI", "right": "QUAY PHẢI" };
            showSuccess(`Đăng ký tư thế ${poseLabelMap[pose]} thành công!`, "Hoàn tất");
                ...
                <div className="p-6 lg:p-8">
                    <div className="max-w-5xl mx-auto flex flex-col gap-6">
                        <nav className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mb-2 font-display">
                            ...
                            <span className="material-symbols-outlined text-xs">chevron_right</span>
                            <span className="text-slate-900 dark:text-slate-200 font-medium">Đăng ký sinh trắc học</span>
                        </nav>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-8">
                        <div className="text-center mb-10">
                            ...
                            <p className="text-slate-500 font-medium">
                                Đang đăng ký sinh trắc học cho: <strong className="text-slate-900 dark:text-slate-200">{user?.fullName || user?.userName || `ID: ${id}`}</strong>
                            </p>
                        </div>
                            ...
                            {[1, 2, 3].map(step => (
                                <div key={step} className="flex items-center gap-3 flex-1">
                                    ...
                                    <div className="flex flex-col">
                                        <span className={`text-[10px] uppercase tracking-tighter font-bold ${step <= currentStep ? "text-primary" : "text-slate-400"}`}>Tư thế {step}</span>
                                        <span className={`text-xs font-bold leading-none ${step <= currentStep ? "text-slate-900 dark:text-slate-100" : "text-slate-400"}`}>
                                            {step === 1 ? "Chính diện" : step === 2 ? "Trái" : "Phải"}
                                        </span>
                                    </div>
                            ...
                            <div className="md:col-span-7 space-y-4">
                                <div className="relative aspect-square md:aspect-[4/3] rounded-3xl overflow-hidden bg-slate-900 shadow-2xl ring-4 ring-primary/5">
                                    ...
                                    <div className="absolute top-6 right-6 px-4 py-1.5 rounded-full bg-black/40 backdrop-blur-md border border-white/10 text-white text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                                        <span className="size-2 bg-primary rounded-full animate-pulse" /> TRỰC TIẾP
                                    </div>
                                </div>
                            ...
                            <div className="md:col-span-5 flex flex-col gap-6">
                                <div className="bg-slate-50 dark:bg-slate-800/40 rounded-2xl p-6 border border-slate-100 dark:border-slate-800">
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <h3 className="font-black text-slate-900 dark:text-white uppercase tracking-tight">Hướng dẫn</h3>
                                    </div>
                                    ...
                                    <div className="mt-6 space-y-3">
                                        {[
                                            { icon: "wb_sunny", text: "Đảm bảo ánh sáng đủ và đều" },
                                            { icon: "face", text: "Giữ nét mặt tự nhiên" },
                                            { icon: "no_photography", text: "Tháo kính hoặc khẩu trang" }
                                        ].map((item, idx) => (
