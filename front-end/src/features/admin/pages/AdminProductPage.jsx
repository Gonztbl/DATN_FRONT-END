...
export default function AdminProductPage() {
    ...
    const handleImageChange = (e) => {
        ...
        if (file) {
            if (file.size > 2 * 1024 * 1024) {
                showWarning("Ảnh quá lớn", "Kích thước ảnh không được vượt quá 2MB");
                e.target.value = '';
                return;
            }
            const allowedTypes = ['image/jpeg', 'image/png', 'image/webp'];
            if (!allowedTypes.includes(file.type)) {
                showWarning("Định dạng không hỗ trợ", "Chỉ chấp nhận ảnh JPG, PNG hoặc WEBP");
                e.target.value = '';
                return;
            }
    ...
    const handleSubmit = async (e) => {
        ...
        if (!payload.name || !payload.price || !payload.restaurantId || !payload.categoryId) {
    ...
    const handleDelete = async (id) => {
        const result = await showConfirm("Xác nhận xóa", "Bạn có chắc chắn muốn xóa món ăn này không?");
